...
interface SceneProps {
  addedModels: AssetConfig[];  // New prop to accept added models
}

export const Scene: React.FC<SceneProps> = ({ addedModels }) => {
  ...
  return (
    <Canvas camera={{ position: [0, 10, 10], fov: 50 }}>
      ...
      {addedModels.map((model, index) => (
        <group key={index}>
          <Model modelPath={model.modelPath} scale={model.scale} />
          <TransformControls ref={transformControlRef} mode="translate">
